...
from data_loader import get_segmentation_dataset, Simulation
    ...
    def train(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.epochs):
            ...
            for i, (images, targets) in enumerate(self.train_loader):
                ...
                loss = self.criterion(outputs[0], targets.long())
...
def save_checkpoint(model, args, is_best=False, epoch = 0):
    print("Save Checkpoint")
    directory = os.path.expanduser(args.save_folder)
    if not os.path.exists(directory):
        os.makedirs(directory)
    if is_best:
        best_filename = '{}_{}_best_model.pth'.format(args.model, args.dataset + " " + str(epoch))
        best_filename = os.path.join(directory, best_filename)
        torch.save(model.state_dict(), best_filename)
    else:
      filename = '{}_{}.pth'.format(args.model, args.dataset + " " + str(epoch))
